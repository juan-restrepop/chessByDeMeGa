...
## Colors for display
white_square_rgb = (145, 120, 0)
black_square_rgb = (78, 60, 18)
...
def screen_big_empty_board(Board, square_width=5, square_height=2):
    screen.clear()
    ...
    for i in range(8):
        for j in range(8):
            screen.addstr(0, 1 + j_0 + j*square_width, board_columns[j])
            screen.addstr(i_0 + i * square_height, 0, board_lines[i])
            color = Board.get_square_color(i, j) + 1
            for k in range(square_height):
                screen.addstr(i_0 + i*square_height + k, j_0 + j * square_width, ' '*square_width, curses.color_pair(color))

    screen.move(height - 1, width - 1)


def board_coord_to_big_screen_coords(i, j, square_width=5, square_height=2):
    I = i * square_height
    J = j * square_width + 2
...
def print_big_cursed_board(Board):

    screen_big_empty_board(Board)

    i_0, j_0 = 1, 2
    for piece in Board.get_all_pieces():
        i, j = piece.coordinates
        if piece.color == 'w':
            I, J = board_coord_to_big_screen_coords(i, j)
            I = I + 1
        else:
            I, J = board_coord_to_big_screen_coords(i, j)
        color = get_piece_color(piece)
